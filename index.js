...
module.exports =
{
    decode : function(jmbg){
      if(this.isValid(jmbg)){
        ...
        return {err: null, data: {
          year, month: month + 1, day, region: region.label,
          place: region.regions[pr],
          gender
        }}
      }else return {err:'Invalid JMBG', data: null}
    },
    isValid : function(jmbg){
      if(jmbg.length !== 13) return false
      else {
        const controlNmb = getControlNmb(jmbg)
        if(controlNmb !== Number(jmbg.charAt(jmbg.length-1))) return false
        else {
    ...
    controlNumber: function(jmbg){
      if(jmbg.length >= 12 && jmbg.length <= 13) return {err:'Invalid JMBG', data: null}
      else return {err:null, data: getControlNmb(jmbg)} 
    }
}
